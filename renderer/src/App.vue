<script setup>
  ...
  import unibox from './utils/unibox.js'
  const $ = (query) => {
  ...
  const newTabUrl = ref('https://limestart.cn/')
  const searchUrl = ref('https://cn.bing.com/search?q=')
  const targetUrl = ref('')
  ...
  const popMenu = ref('')
  const useragent = ref(navigator.userAgent.replace(/Electron\/\d*(\.\d)*\s/, ""))
  console.log("UA: " + navigator.userAgent.replace(/Electron\/\d*(\.\d)*\s/, ""))
  ...
  const initWebview = (id) => {
    ...
    $("#webview" + id).addEventListener('did-stop-loading', () => {
      ...
      //$("#webview" + id).executeJavaScript("window.open=(e)=>{//console.log(e)}")
      //$("#webview" + id).openDevTools()
      doAddressInputRecover()
      canWebBackForward()
    })
  ...
  const onContextMenuMainClick = (action, data) => {
    //console.log(tabContextMenuIndex.value)
  ...
  const doWebZoom = (action) => {
    ...
    //console.log(action, $("#webview" + activeTabsId.value).getZoomFactor())
  ...
  const init = () => {
    addTab('https://limestart.cn/')//https://cn.bing.com/search?q=测试
    removeTab('')
  }
  ...
  onMounted(() => {
    ...
    sendNotification({
      ...
      message: '<b>夏季测试版本 23w24c</b> (Ryokin 0.0.1)',
      //duration: 0,
      offset: 80
    })
  })
...
<template>
  ...
  <el-row class="toolbar">
    ...
    <el-input id="addressBar" v-model="webTabs[getTabIndex(activeTabsId)].urlInput" @focus="onAddressBarFocus"
      @click.right="onAddressBarContextMenu" @keydown.esc="doAddressInputRecover" @keydown.ctrl.enter="doWebGo('force')"
      @keydown.enter.exact="doWebGo()" class="addressBar" :style="'width:'+';margin-left: 5px;margin-right: 5px;'">
      <template #append style="background-color: var(--el-fill-color-blank);">
        ...
        <el-button @click="showBuildingTip" size="small" text :icon="Star" class="addressBarButton disabled"></el-button>
      </template>
    </el-input>
